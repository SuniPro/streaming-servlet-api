...
public class LeagueController {
    ...
    @PostMapping("create")
    public Flux<PerfectSportsLeagueDTO> createLeagueInfo(@RequestBody List<PerfectSportsLeagueDTO> perfectSportsLeagueDTOList) {

        return sportsLeagueInfoService.createSportsLeague(perfectSportsLeagueDTOList);
    }

    @GetMapping("read/{sportsType}")
    public Mono<List<PerfectSportsLeagueDTO>> getLeagueInfoBySportsType(@PathVariable SportsType sportsType, @RequestParam int page, @RequestParam int size){
